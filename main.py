...
def test(books: list[Book]):
    ...
    spring_server_url = "http://localhost:9039/hi"  # Spring 서버의 URL로 변경
    print(data)
...
def hi(book: Book):  # Fix 2: Use 'book' instead of 'Book'
    completion = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "user", "content": book.content + " 이 문장이 유해한 단어나 욕설을 포함한다면, 0을 출력해줘 다른 이상한 말은 석지말고, 유해한 단어나 욕설이 없다면 1을 출력해줘 다른 이상한 말은 석지말고, 답변에 숫자만 출력해"}
        ]
    )
